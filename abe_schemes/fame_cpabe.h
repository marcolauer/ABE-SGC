#ifndef MASTER_FAME_CPABE_H
#define MASTER_FAME_CPABE_H
...
namespace fame_cpabe {
    ...
    /**
     * Runs the setup algorithm of the FAME CP-ABE scheme.
    ...
    /**
     * Runs the key generation algorithm of the FAME CP-ABE scheme.
     ...
     */
    void key_generation(secret_key& sk, bn_t order, const std::vector<int>& identity,
                        const public_key& pk, const master_key& mk);

    /**
     * Runs the encryption algorithm of the FAME CP-ABE scheme.
     ...
     * @param[in] pk the ABE public key.
     */
    void encryption(ciphertext& ct, bn_t order, gt_t message, TTree *policy,
                    const public_key& pk);

    /**
     * Runs the decryption algorithm of the FAME CP-ABE scheme.
    ...
    /**
     * Frees the allocated content in the ABE master key data structure of the FAME CP-ABE scheme.
     * @param[out] mk the (empty) ABE master key.
     */
    void free_master_key(master_key& mk);

    /**
     * Frees the allocated content in the ABE public key data structure of the FAME CP-ABE scheme.
     * @param[out] pk the (empty) ABE public key.
     */
    void free_public_key(public_key& pk);

    /**
     * Frees the allocated content in the ABE secret key data structure of the FAME CP-ABE scheme.
     * @param[out] sk the (empty) ABE secret key.
     */
    void free_secret_key(secret_key& sk);

    /**
     * Frees the allocated content in the ABE ciphertext data structure of the FAME CP-ABE scheme.
    ...
    /**
     * Serializes the ABE master key data structure of the FAME CP-ABE scheme to bytes.
    ...
    /**
     * Deserializes the ABE master key data structure of the FAME CP-ABE scheme from bytes.
    ...
    /**
     * Serializes the ABE public key data structure of the FAME CP-ABE scheme to bytes.
    ...
    /**
     * Deserializes the ABE public key data structure of the FAME CP-ABE scheme from bytes.
    ...
    /**
     * Serializes the ABE secret key data structure of the FAME CP-ABE scheme to bytes.
    ...
    void serialize_sk(std::vector<unsigned char>& data, const secret_key& sk) {
        ...
        serialize_g1_t_vector<uint8_t>(data, sk.skprimes);
    }

    /**
     * Deserializes the ABE secret key data structure of the FAME CP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    secret_key deserialize_sk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        sk.skprimes = deserialize_g1_t_vector<uint8_t>(data, offset_ptr);
        return sk;
    }

    /**
     * Serializes the ABE ciphertext data structure of the FAME CP-ABE scheme to bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    /**
     * Deserializes the ABE ciphertext data structure of the FAME CP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
