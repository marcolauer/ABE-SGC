#ifndef MASTER_KPABE_LARGE_S_H
#define MASTER_KPABE_LARGE_S_H
...
namespace kpabe_large_s {
    ...
    /**
     * Runs the setup algorithm of the large universe KP-ABE scheme.
    ...
    /**
     * Runs the key generation algorithm of the large universe KP-ABE scheme.
    ...
    /**
     * Runs the encryption algorithm of the large universe KP-ABE scheme.
    ...
    /**
     * Runs the decryption algorithm of the large universe KP-ABE scheme.
    ...
    /**
     * Frees the allocated content in the ABE master key data structure of the large universe KP-ABE scheme.
     * @param[out] mk the (empty) ABE master key.
     */
    void free_master_key(const master_key& mk);

    /**
     * Frees the allocated content in the ABE public key data structure of the large universe KP-ABE scheme.
     * @param[out] pk the (empty) ABE public key.
     */
    void free_public_key(public_key& pk);

    /**
     * Frees the allocated content in the ABE secret key data structure of the large universe KP-ABE scheme.
     * @param[out] sk the (empty) ABE secret key.
     */
    void free_secret_key(secret_key& sk);

    /**
     * Frees the allocated content in the ABE ciphertext data structure of the large universe KP-ABE scheme.
    ...
    /**
     * Serializes the ABE master key data structure of the large universe KP-ABE scheme to bytes.
    ...
    /**
     * Deserializes the ABE master key data structure of the large universe KP-ABE scheme from bytes.
    ...
    /**
     * Serializes the ABE public key data structure of the large universe KP-ABE scheme to bytes.
    ...
    void serialize_pk(std::vector<unsigned char>& data, const public_key& pk) {
        ...
        serialize_g1_t_vector<ATTR_TYPE>(data, pk.ts);
    }

    /**
     * Deserializes the ABE public key data structure of the large universe KP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
    ...
    public_key deserialize_pk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        pk.ts = deserialize_g1_t_vector<ATTR_TYPE>(data, offset_ptr);
        return pk;
    }

    /**
     * Serializes the ABE secret key data structure of the large universe KP-ABE scheme to bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
    ...
    void serialize_sk(std::vector<unsigned char>& data, const secret_key& sk) {
        ...
        serialize_tattribute_g2_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, sk.Rs);
    }

    /**
     * Deserializes the ABE secret key data structure of the large universe KP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    secret_key deserialize_sk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        sk.Rs = deserialize_tattribute_g2_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        return sk;
    }

    /**
     * Serializes the ABE ciphertext data structure of the large universe KP-ABE scheme to bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    void serialize_ct(std::vector<unsigned char>& data, const ciphertext& ct) {
        ...
        serialize_g2_t(data, *ct.Eprimeprime);
        const std::map<ATTR_TYPE, g1_t *> Es(ct.Es.begin(), ct.Es.end());
        serialize_int_g1_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, Es);
    }

    /**
     * Deserializes the ABE ciphertext data structure of the large universe KP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    ciphertext deserialize_ct(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        ct.Eprimeprime = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*ct.Eprimeprime);
        deserialize_g2_t(*ct.Eprimeprime, data, offset_ptr);
        std::map<ATTR_TYPE, g1_t *> Es = deserialize_int_g1_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        ct.Es = std::map<int, g1_t *>(Es.begin(), Es.end());
        return ct;
    }
}

#endif //MASTER_KPABE_LARGE_S_H
