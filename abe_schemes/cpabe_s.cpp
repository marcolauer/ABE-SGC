...
namespace cpabe_s {
    void setup(master_key& mk, public_key& pk, bn_t order) {
        ...
        // Public Parameter h
        pk.h = static_cast<g1_t *>(malloc(sizeof(g1_t)));
        g1_util_null_init(*pk.h);
        g1_mul_gen(*pk.h, *beta);
        // Master Key
        mk.g_alpha = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*mk.g_alpha);
        g2_mul_gen(*mk.g_alpha, alpha);
    ...
    void key_generation(secret_key& sk, bn_t order, const std::vector<int>& identity, const master_key& mk) {
        ...
        g2_t grG1;
        g2_util_null_init(grG1);
        g1_t grG2;
        g1_util_null_init(grG2);
        bn_rand_mod(r, order);
        g2_mul_gen(grG1, r);
        g1_mul_gen(grG2, r);

        sk.D = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*sk.D);
        g2_add(*sk.D, grG1, *mk.g_alpha);
        g2_mul(*sk.D, *sk.D, *mk.beta_inv);
        for (const auto attr : identity) {
            bn_rand_mod(rj, order);
            auto Di = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*Di);
            g1_map(*Di, static_cast<const uint8_t *>(static_cast<void *>(const_cast<int *>(&attr))), sizeof(int));
            g1_mul(*Di, *Di, rj);
            g1_add(*Di, grG2, *Di);
            sk.Ds[attr] = Di;
            auto Dprimei = static_cast<g2_t *>(malloc(sizeof(g2_t)));
            g2_util_null_init(*Dprimei);
            g2_mul_gen(*Dprimei, rj);
    ...
    void encryption(ciphertext& ct, bn_t order, gt_t message, TTree *policy, const public_key& pk) {
        ...
        // C
        ct.C = static_cast<g1_t *>(malloc(sizeof(g1_t)));
        g1_util_null_init(*ct.C);
        g1_mul(*ct.C, *pk.h, s);

        for (const auto& [tattr, exponent] : shares) {
            int attr = tattr.get_attribute();
            auto Ci = static_cast<g2_t *>(malloc(sizeof(g2_t)));
            g2_util_null_init(*Ci);
            g2_mul_gen(*Ci, *exponent);
            ct.Cs[tattr] = Ci;
            auto Cprimei = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*Cprimei);
            g1_map(*Cprimei, static_cast<const uint8_t *>(static_cast<void *>(&attr)), sizeof(int));
            g1_mul(*Cprimei, *Cprimei, *exponent);
    ...
    void decryption(gt_t message, bn_t order, const ciphertext& ct, const secret_key& sk) {
        ...
        for (int i = 0; i < num_attributes; ++i) {
            ...
            if (!bn_is_zero(*coeff)) {
                ...
                pc_map(pairing_array[count], *sk.Ds.at(attr), *ct.Cs.at(tattr));
                pc_map(temp, *ct.Cprimes.at(tattr), *sk.Dprimes.at(attr));
    ...
    void serialize_mk(std::vector<unsigned char>& data, const master_key& mk) {
        ...
        serialize_g2_t(data, *mk.g_alpha);
    }
    ...
    master_key deserialize_mk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        deserialize_g2_t(*mk.g_alpha, data, offset_ptr);
        return mk;
    }

    void serialize_pk(std::vector<unsigned char>& data, const public_key& pk) {
        serialize_g1_t(data, *pk.h);
    ...
    public_key deserialize_pk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        pk.h = static_cast<g1_t *>(malloc(sizeof(g1_t)));
        g1_util_null_init(*pk.h);
        deserialize_g1_t(*pk.h, data, offset_ptr);
        pk.egg_alpha = static_cast<gt_t *>(malloc(sizeof(gt_t)));
        gt_util_null_init(*pk.egg_alpha);
