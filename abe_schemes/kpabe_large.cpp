...
namespace kpabe_large {
    ...
    void key_generation(secret_key& sk, bn_t order, TTree *policy, const public_key& pk, const master_key& mk) {
        ...
        for (const auto& [tattr, q] : shares) {
            ...
            g2_mul(*Di, *Di, temp);
            // Rs
            auto Ri = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*Ri);
            g1_mul_gen(*Ri, temp);
            sk.Rs[tattr] = Ri;
            // g2^q(attr)
            g2_mul_fix(temp_g2, g2_pre, *q);
    ...
    void decryption(gt_t message, bn_t order, const ciphertext& ct, const secret_key& sk) {
        ...
        for (int i = 0; i < num_attributes; ++i) {
            ...
            if (!bn_is_zero(*coeff)) {
                ...
                pc_map(pairing_array[count], *sk.Rs.at(tattr), *ct.Es.at(attr));
                pc_map(temp, *ct.Eprimeprime, *sk.Ds.at(tattr));
