...
namespace cpabbe {
    ...
    void key_generation(secret_key& sk, bn_t order, const int id, const std::vector<int>& V, const std::vector<int>& Z,
        ...
        sk.D4s = std::vector<g1_t *>(pk.N1+1);
        g1_t v_hs[n2];
        for (int i = 0; i < n2; ++i) {
        ...
        for (int i = 0; i < pk.N1+1; ++i) {
            auto D4i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*D4i);
            g1_set_infty(*D4i);
            for (int j = 0; j < n2; ++j) {
                g1_add(*D4i, *D4i, v_hs[j]);
            }
            g1_mul(*D4i, *D4i, temp1);
            sk.D4s[i] = D4i;
            if (i != pk.N1) {
                for (int j = 0; j < n2; ++j) {
        ...
        // D5
        bn_rand_mod(temp1, order);
        sk.D5s = std::vector<g1_t *>(pk.N1+1);
        g1_t z_hs[n3];
        for (int i = 0; i < n3; ++i) {
        ...
        for (int i = 0; i < pk.N1+1; ++i) {
            auto D5i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*D5i);
            g1_set_infty(*D5i);
            for (int j = 0; j < n3; ++j) {
                g1_add(*D5i, *D5i, z_hs[j]);
            }
            g1_mul(*D5i, *D5i, temp1);
            sk.D5s[i] = D5i;
            if (i != pk.N1) {
                for (int j = 0; j < n3; ++j) {
    ...
    void encryption(ciphertext& ct, bn_t order, gt_t message, const std::vector<int>& S, const std::vector<int>& J,
        ...
        bn_t c3_exps[n2];
        for (int i = 0; i < n2; ++i) {
            g2_util_null_init(c3_hs[i]);
            g2_copy(c3_hs[i], *pk.hs_g2[V[i]-1]);
        ...
        ct.C3 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*ct.C3);
        g2_util_mul_sim_lot(*ct.C3, c3_hs, c3_exps, n2);
        g2_add(*ct.C3, *pk.v0, *ct.C3);
        g2_mul(*ct.C3, *ct.C3, r);
        for (int i = 0; i < n2; ++i) {
            g2_free(c3_hs[i]);
            bn_free(c3_exps[i]);
        }
        ...
        bn_t c4_exps[n3];
        for (int i = 0; i < n3; ++i) {
            g2_util_null_init(c4_hs[i]);
            g2_copy(c4_hs[i], *pk.hs_g2[Z[i]-1]);
        ...
        ct.C4 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*ct.C4);
        g2_util_mul_sim_lot(*ct.C4, c4_hs, c4_exps, n3);
        g2_add(*ct.C4, *pk.v1, *ct.C4);
        g2_mul(*ct.C4, *ct.C4, r);
        for (int i = 0; i < n3; ++i) {
            g2_free(c4_hs[i]);
            bn_free(c4_exps[i]);
        }
    ...
    void decryption(gt_t message, bn_t order, const ciphertext& ct, const secret_key& sk) {
        ...
        for (int i = 0; i < n1pp; ++i) {
            ...
            g1_util_null_init(D4s[i]);
            g1_util_null_init(D5s[i]);
            bn_copy(as_array[i], *as[i]);
            g1_copy(D4s[i], *sk.D4s[i]);
            g1_copy(D5s[i], *sk.D5s[i]);
        }
        g1_util_null_init(g1s_num[1]);
        g1_util_mul_sim_lot(g1s_num[1], D4s, as_array, n1pp);
        g1_util_null_init(g1s_num[2]);
        g1_util_mul_sim_lot(g1s_num[2], D5s, as_array, n1pp);
        ...
        g2_copy(g2s_den[1], *ct.C3);
        g2_util_null_init(g2s_den[2]);
        g2_copy(g2s_den[2], *ct.C4);
