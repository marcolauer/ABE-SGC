#ifndef MASTER_FABEO_CPABE_H
#define MASTER_FABEO_CPABE_H
...
namespace fabeo_cpabe {
    ...
    struct ciphertext {
        ...
        g2_t *ct1;
        std::vector<g2_t *> ct2;
        std::map<MSPAttribute, g1_t *> ct3;
        gt_t *d{};
    };


    /**
     * Runs the setup algorithm of the FABEO CP-ABE scheme.
    ...
    /**
     * Runs the key generation algorithm of the FABEO CP-ABE scheme.
    ...
    /**
     * Runs the encryption algorithm of the FABEO CP-ABE scheme.
    ...
    /**
     * Runs the decryption algorithm of the FABEO CP-ABE scheme.
    ...
    /**
     * Frees the allocated content in the ABE master key data structure of the FABEO CP-ABE scheme.
     * @param[out] mk the (empty) ABE master key.
     */
    void free_master_key(const master_key& mk);

    /**
     * Frees the allocated content in the ABE public key data structure of the FABEO CP-ABE scheme.
     * @param[out] pk the (empty) ABE public key.
     */
    void free_public_key(const public_key& pk);

    /**
     * Frees the allocated content in the ABE secret key data structure of the FABEO CP-ABE scheme.
     * @param[out] sk the (empty) ABE secret key.
     */
    void free_secret_key(secret_key& sk);

    /**
     * Frees the allocated content in the ABE ciphertext data structure of the FABEO CP-ABE scheme.
    ...
    /**
     * Serializes the ABE master key data structure of the FABEO CP-ABE scheme to bytes.
    ...
    /**
     * Deserializes the ABE master key data structure of the FABEO CP-ABE scheme from bytes.
    ...
    /**
     * Serializes the ABE public key data structure of the FABEO CP-ABE scheme to bytes.
    ...
    /**
     * Deserializes the ABE public key data structure of the FABEO CP-ABE scheme from bytes.
    ...
    /**
     * Serializes the ABE secret key data structure of the FABEO CP-ABE scheme to bytes.
    ...
    void serialize_sk(std::vector<unsigned char>& data, const secret_key& sk) {
        ...
        serialize_g1_t(data, *sk.sk1);
        const std::map<ATTR_TYPE, g1_t *> sk2(sk.sk2.begin(), sk.sk2.end());
        serialize_int_g1_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, sk2);
        serialize_g2_t(data, *sk.sk3);
    }

    /**
     * Deserializes the ABE secret key data structure of the FABEO CP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    secret_key deserialize_sk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        deserialize_g1_t(*sk.sk1, data, offset_ptr);
        std::map<ATTR_TYPE, g1_t *> sk2 = deserialize_int_g1_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        sk.sk2 = std::map<int, g1_t *>(sk2.begin(), sk2.end());
        sk.sk3 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*sk.sk3);
        deserialize_g2_t(*sk.sk3, data, offset_ptr);
        return sk;
    }

    /**
     * Serializes the ABE ciphertext data structure of the FABEO CP-ABE scheme to bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    void serialize_ct(std::vector<unsigned char>& data, const ciphertext& ct) {
        ...
        serialize_int<IDENTITY_SIZE_TYPE>(data, ct.tau);
        serialize_g2_t(data, *ct.ct1);
        serialize_g2_t_vector<IDENTITY_SIZE_TYPE>(data, ct.ct2);
        serialize_msp_attribute_g1_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, ct.ct3);
        serialize_gt_t(data, *ct.d);
    }

    /**
     * Deserializes the ABE ciphertext data structure of the FABEO CP-ABE scheme from bytes.
     * @tparam ATTR_TYPE the data type used for the attributes (e.g. uint8_t, uint16_t, ...).
     * @tparam IDENTITY_SIZE_TYPE the data type used for serializing the length of the identities / access structures
     *      (e.g. uint8_t, uint16_t, ...).
    ...
    ciphertext deserialize_ct(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        ct.tau = deserialize_int<IDENTITY_SIZE_TYPE>(data, offset_ptr);
        ct.ct1 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*ct.ct1);
        deserialize_g2_t(*ct.ct1, data, offset_ptr);
        ct.ct2 = deserialize_g2_t_vector<IDENTITY_SIZE_TYPE>(data, offset_ptr);
        ct.ct3 = deserialize_msp_attribute_g1_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
