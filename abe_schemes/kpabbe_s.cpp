...
namespace kpabbe_s {
    void setup(master_key& mk, public_key& pk, bn_t order, const int n, const int N, const int N1) {
        ...
        pk.v1s.reserve(N1);
        for (int i = 0; i < N1; ++i) {
            auto *v0i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*v0i);
            g1_mul(*v0i, *pk.v0, *mk.xs[i]);
            pk.v0s.push_back(v0i);
            auto *v1i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*v1i);
            g1_mul(*v1i, *pk.v1, *mk.xs[i]);
    ...
    void key_generation(secret_key& sk, bn_t order, const int id, const std::vector<int>& J, const std::vector<int>& V,
        ...
        bn_t d5_exps[n3];
        for (int i = 0; i < n3; ++i) {
            g2_util_null_init(d5_hs[i]);
            g2_copy(d5_hs[i], *pk.hs_g2[Z[i]-1]);
        ...
        sk.D5 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*sk.D5);
        g2_util_mul_sim_lot(*sk.D5, d5_hs, d5_exps, n3);
        for (int i = 0; i < n3; ++i) {
            g2_free(d5_hs[i]);
            bn_free(d5_exps[i]);
        }
        ...
        bn_t d4_exps[n2];
        for (int i = 0; i < n2; ++i) {
            g2_util_null_init(d4_hs[i]);
            g2_copy(d4_hs[i], *pk.hs_g2[V[i]-1]);
        ...
        sk.D4 = static_cast<g2_t *>(malloc(sizeof(g2_t)));
        g2_util_null_init(*sk.D4);
        g2_util_mul_sim_lot(*sk.D4, d4_hs, d4_exps, n2);
        for (int i = 0; i < n2; ++i) {
            g2_free(d4_hs[i]);
            bn_free(d4_exps[i]);
        }
    ...
    void encryption(ciphertext& ct, bn_t order, gt_t message, const std::vector<int>& S, const std::vector<int>& V,
        ...
        // C3
        ct.C3s = std::vector<g1_t *>(pk.N1+1);
        g1_t v_hs[n2];
        for (int i = 0; i < n2; ++i) {
            g1_util_null_init(v_hs[i]);
            g1_copy(v_hs[i], *pk.hs_g1[V[i]-1]);
        }
        auto C30 = static_cast<g1_t *>(malloc(sizeof(g1_t)));
        g1_util_null_init(*C30);
        g1_set_infty(*C30);
        for (int i = 0; i < n2; ++i) {
            g1_add(*C30, *C30, v_hs[i]);
        }
        g1_add(*C30, *pk.v0, *C30);
        g1_mul(*C30, *C30, r);
        ct.C3s[0] = C30;
        for (int i = 0; i < pk.N1; ++i) {
            for (int j = 0; j < n2; ++j) {
                g1_util_mul_int_mod(v_hs[j], v_hs[j], V[j], order);
            }
            auto C3i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*C3i);
            g1_set_infty(*C3i);
            for (int j = 0; j < n2; ++j) {
                g1_add(*C3i, *C3i, v_hs[j]);
            }
            g1_add(*C3i, *pk.v0s[i], *C3i);
            g1_mul(*C3i, *C3i, r);
            ct.C3s[i+1] = C3i;
        }
        // C4
        ct.C4s = std::vector<g1_t *>(pk.N1+1);
        g1_t z_hs[n3];
        for (int i = 0; i < n3; ++i) {
            g1_util_null_init(z_hs[i]);
            g1_copy(z_hs[i], *pk.hs_g1[Z[i]-1]);
        }
        auto C40 = static_cast<g1_t *>(malloc(sizeof(g1_t)));
        g1_util_null_init(*C40);
        g1_set_infty(*C40);
        for (int i = 0; i < n3; ++i) {
            g1_add(*C40, *C40, z_hs[i]);
        }
        g1_add(*C40, *pk.v1, *C40);
        g1_mul(*C40, *C40, r);
        ct.C4s[0] = C40;
        for (int i = 0; i < pk.N1; ++i) {
            for (int j = 0; j < n3; ++j) {
                g1_util_mul_int_mod(z_hs[j], z_hs[j], Z[j], order);
            }
            auto C4i = static_cast<g1_t *>(malloc(sizeof(g1_t)));
            g1_util_null_init(*C4i);
            g1_set_infty(*C4i);
            for (int j = 0; j < n3; ++j) {
                g1_add(*C4i, *C4i, z_hs[j]);
            }
            g1_add(*C4i, *pk.v1s[i], *C4i);
            g1_mul(*C4i, *C4i, r);
            ct.C4s[i+1] = C4i;
        }
        bn_free(r);
    ...
    void decryption(gt_t message, bn_t order, const ciphertext& ct, const secret_key& sk) {
        ...
        g2_copy(g2s_num[1], *sk.D4);
        g2_util_null_init(g2s_num[2]);
        g2_copy(g2s_num[2], *sk.D5);
        ...
        for (int i = 0; i < n1pp; ++i) {
            ...
            g1_util_null_init(C3s[i]);
            g1_util_null_init(C4s[i]);
            bn_copy(as_array[i], *as[i]);
            g1_copy(C3s[i], *ct.C3s[i]);
            g1_copy(C4s[i], *ct.C4s[i]);
        }
        g1_util_null_init(g1s_den[1]);
        g1_util_mul_sim_lot(g1s_den[1], C3s, as_array, n1pp);
        g1_util_null_init(g1s_den[2]);
        g1_util_mul_sim_lot(g1s_den[2], C4s, as_array, n1pp);
