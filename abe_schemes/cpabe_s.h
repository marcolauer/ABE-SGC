#ifndef MASTER_CPABE_S_H
#define MASTER_CPABE_S_H
...
namespace cpabe_s {
    ...
    struct ciphertext {
        ...
        gt_t *C_circum{};
        g1_t *C{};
        std::map<TAttribute, g2_t *> Cs;
        std::map<TAttribute, g1_t *> Cprimes;
    };
    ...
    void serialize_sk(std::vector<unsigned char>& data, const secret_key& sk) {
        ...
        serialize_g2_t(data, *sk.D);
        const std::map<ATTR_TYPE, g1_t *> Ds(sk.Ds.begin(), sk.Ds.end());
        serialize_int_g1_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, Ds);
        const std::map<ATTR_TYPE, g2_t *> Dprimes(sk.Dprimes.begin(), sk.Dprimes.end());
        serialize_int_g2_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, Dprimes);
    }
    ...
    secret_key deserialize_sk(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        deserialize_g2_t(*sk.D, data, offset_ptr);
        ...
        std::map<ATTR_TYPE, g2_t *> Dprimes = deserialize_int_g2_t_map<ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        sk.Dprimes = std::map<int, g2_t *>(Dprimes.begin(), Dprimes.end());
        return sk;
    }
    ...
    void serialize_ct(std::vector<unsigned char>& data, const ciphertext& ct) {
        ...
        serialize_g1_t(data, *ct.C);
        serialize_tattribute_g2_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, ct.Cs);
        serialize_tattribute_g1_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, ct.Cprimes);
    }
    ...
    ciphertext deserialize_ct(const std::vector<unsigned char>& data, int *offset_ptr) {
        ...
        deserialize_g1_t(*ct.C, data, offset_ptr);
        ct.Cs = deserialize_tattribute_g2_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        ct.Cprimes = deserialize_tattribute_g1_t_map<ATTR_TYPE, ATTR_TYPE, IDENTITY_SIZE_TYPE>(data, offset_ptr);
        return ct;
    }
}

#endif //MASTER_CPABE_S_H
